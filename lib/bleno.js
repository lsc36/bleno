...
Bleno.prototype.setOobTk = function(oobTk) {
  if (oobTk.constructor === String) {
    oobTk = Buffer.from(oobTk, 'hex');
  } else if (oobTk.constructor !== Buffer) {
    throw new Error('oobTk should be either hex string or Buffer object');
  }
  if (oobTk.length !== 16) {
    throw new Error('oobTk should be 128-bit long');
  }
  this._bindings.setOobTk(oobTk);
};
