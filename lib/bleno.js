...
Bleno.prototype.setTk = function(tk) {
  if (tk.constructor === String) {
    tk = Buffer.from(tk, 'hex');
  } else if (tk.constructor !== Buffer) {
    throw new Error('tk should be either hex string or Buffer object');
  }
  if (tk.length !== 16) {
    throw new Error('tk should be 128-bit long');
  }
  this._bindings.setTk(tk);
};

Bleno.prototype.setPairingFeature = function(ioCap, isOob, authReq) {
  this._bindings.setPairingFeature(ioCap, isOob, authReq);
};
